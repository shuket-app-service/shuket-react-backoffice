...
export const getBannerList = createAsyncThunk(
  GET_BANNER_LIST,
async ({page, per_page, filter_status, filter_order, image_cate, key_value}, { rejectWithValue }) => {
  try {
    const response = await apiAuth.get(`${GET_BANNER_LIST}?page=${page}&per_page=${per_page}&filter_status=${filter_status}&filter_order=${filter_order}&image_cate=${image_cate}&key_value=${key_value}`);
    return response.data.data;
  } catch (err) {
    return rejectWithValue(err.message);
  }
